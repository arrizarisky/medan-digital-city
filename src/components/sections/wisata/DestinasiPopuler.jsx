...
// ── Card dengan fade-up individual ───────────────────────────
function DestinasiCard({ item, viewDetailBtn, index }) {
  return (
    <motion.article
      className="group flex flex-col bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl border border-gray-100 transition-shadow duration-300 hover:-translate-y-1"
    ...
    >
      ...
      <div className="flex flex-col flex-1 p-5 gap-3">
        <div className="flex-1">
          <h3 className="text-base font-bold text-gray-900 leading-snug mb-1.5">{item.name}</h3>
    ...
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className="flex items-end justify-between mb-8 gap-4">
          ...
          {/* Toggle Show All / Show Less button — desktop */}
          {content.items.length > 3 && (
            ...
            >
              {showAll ? content.viewLess || 'Lihat Sedikit' : content.viewAll}
              <svg
                ...
                className={`transition-transform duration-300 ${showAll ? 'rotate-90' : ''} group-hover:translate-x-0.5`}
              >
