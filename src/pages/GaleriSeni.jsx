...
export default function GaleriSeni() {
  const { lang } = useLanguage();
  const [activeCategory, setActiveCategory] = useState(galeriCategories[lang][0]);
  ...
  useEffect(() => {
    setActiveCategory(galeriCategories[lang][0]);
  }, [lang]);

  // Filter items based on active category
  const filteredItems = useMemo(() => {
    if (activeCategory === galeriCategories[lang][0]) return galeriItems;
    return galeriItems.filter(
      (item) => item.category[lang] === activeCategory || item.koleksi[lang] === activeCategory
    );
  }, [activeCategory, lang]);

  // Compute per-category counts
  const categories = useMemo(() => {
    return galeriCategories[lang].map((name) => ({
      name,
      count:
        name === galeriCategories[lang][0]
          ? galeriItems.length
          : galeriItems.filter((i) => i.category[lang] === name || i.koleksi[lang] === name).length,
    }));
  }, [lang]);
  ...
  return (
    <>
        ...
        <main className="relative z-10">
          ...
          <section className="relative h-screen w-full overflow-hidden flex items-end">
            <div className="absolute inset-0 z-0">
              <img 
                alt={lang === 'id' ? 'Warisan Arsitektur' : 'Heritage Architecture'} 
                className="w-full h-full object-cover brightness-75" 
                src={galeriSeniIstana}
              />
            ...
            <div className="relative z-10 max-w-[1400px] mx-auto px-4 py-8 md:px-12 md:pb-24 md:pt-0 w-full">
              <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
                <motion.div 
                  ...
                  className="max-w-3xl"
                >
                  <span className="inline-block px-4 py-1.5 bg-[#B2533E] text-white text-[10px] font-bold uppercase tracking-[0.2em] mb-4 rounded-full shadow-lg">
                    {lang === 'id' ? 'Eksplorasi Kurasi Utama' : 'Main Curated Exploration'}
                  </span>
                  <h1 className="font-display-lg text-4xl md:text-5xl lg:text-[84px] text-white leading-[1.1] mb-4 md:mb-6">
                    {lang === 'id' ? 'Galeri Karya &\nSeni Pertunjukan' : 'Artworks &\nPerforming Arts Gallery'}
                  </h1>
                  <p className="font-body-lg text-white/80 max-w-xl border-l-2 border-[#B2533E] pl-4 md:pl-6 italic text-[14px] md:text-[18px]">
                    {lang === 'id' 
                      ...
                      : 'A window into the spiritual and aesthetic richness of Deli Land, deeply curated to preserve the nation\'s collective memory.'}
                  </p>
                </motion.div>
              </div>
            </div>
          ...
          <div className="max-w-[1400px] mx-auto px-8 pt-24 pb-16 flex flex-col lg:flex-row gap-16">
            ...
            >
              <div className="sticky top-32">
                <h2 className="font-display-lg text-2xl font-bold text-[#1E3F20] mb-8 pb-4 border-b border-[#c1c8c2]">
                ...
                <div className="mt-20 pt-10 border-t border-[#c1c8c2]">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-[#B2533E] mb-4">
                    {lang === 'id' ? 'Informasi Kurasi' : 'Curatorial Information'}
                  </p>
                  <p className="font-body-md text-[16px] text-[#1b1c1c]/60 leading-relaxed italic">
                    {lang === 'id' 
                      ...
                      : 'Every item in this gallery has been verified by the regional cultural board for its historical authenticity.'}
                  </p>
                </div>
              </div>
            </motion.aside>
            ...
            <div className="flex-grow">
              {filteredItems.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-64 text-[#1b1c1c]/40">
                  <p className="font-display-lg text-2xl mb-2">{lang === 'id' ? 'Belum Ada Koleksi' : 'No Collection Yet'}</p>
                  <p className="font-body-md text-sm">{lang === 'id' ? 'Kategori ini belum memiliki karya yang terdaftar.' : 'This category does not yet have registered works.'}</p>
                </div>
              ) : (
                <motion.div 
                  layout
                          ...
                          <motion.div key={item.id} {...motionProps} className={`md:row-span-2 ${cardBaseClasses}`}>
                            <div className="relative flex-grow min-h-[300px] overflow-hidden">
                              <img 
                                ...
                                src={item.image}
                              />
                              <div className="absolute top-6 left-6 px-3 py-1.5 bg-white/90 backdrop-blur-sm text-[10px] font-bold text-[#1E3F20] uppercase tracking-widest rounded-full">
                                {lang === 'id' ? 'Koleksi' : 'Collection'} {koleksiNum}
                              </div>
                            </div>
                            <div className="p-8 bg-white z-10">
                              <p className="text-[10px] text-[#B2533E] font-bold uppercase tracking-widest mb-1.5">{item.koleksi[lang]}</p>
                              <h3 className="font-display-lg text-xl font-bold text-[#1E3F20] mb-3 leading-tight">{item.title[lang]}</h3>
                              <p className="font-body-md text-[13px] text-[#1b1c1c]/70 leading-relaxed">{item.caption[lang]}</p>
                            </div>
                          </motion.div>
                        );
                      }
                          ...
                          <motion.div key={item.id} {...motionProps} className={`lg:col-span-2 sm:flex-row ${cardBaseClasses}`}>
                            <div className={`relative sm:w-3/5 min-h-[250px] overflow-hidden ${imgLeft ? 'rounded-t-[2rem] sm:rounded-none sm:rounded-l-[2rem]' : 'rounded-b-[2rem] sm:rounded-none sm:rounded-r-[2rem] order-1 sm:order-2'}`}>
                              <img 
                                ...
                                src={item.image}
                              />
                              <div className={`absolute top-6 ${imgLeft ? 'left-6' : 'right-6'} px-3 py-1.5 bg-white/90 backdrop-blur-sm text-[10px] font-bold text-[#1E3F20] uppercase tracking-widest rounded-full`}>
                                {lang === 'id' ? 'Koleksi' : 'Collection'} {koleksiNum}
                              </div>
                            </div>
                            <div className={`p-8 sm:w-2/5 flex flex-col justify-center bg-white z-10 ${imgLeft ? 'border-l border-[#c1c8c2]/10' : 'border-r border-[#c1c8c2]/10 order-2 sm:order-1'}`}>
                              <p className="text-[10px] text-[#B2533E] font-bold uppercase tracking-widest mb-1.5">{item.koleksi[lang]}</p>
                              <h3 className="font-display-lg text-2xl font-bold text-[#1E3F20] mb-3 leading-tight">{item.title[lang]}</h3>
                              <p className="font-body-md text-[14px] text-[#1b1c1c]/70 leading-relaxed italic">{item.caption[lang]}</p>
                            </div>
                          </motion.div>
                        );
                      }
                      ...
                      return (
                        <motion.div key={item.id} {...motionProps} className={`col-span-1 ${cardBaseClasses}`}>
                          ...
                          <div className="p-6 bg-white z-10">
                            <p className="text-[10px] text-[#B2533E] font-bold uppercase tracking-widest mb-1.5">{item.koleksi[lang]}</p>
                            <h3 className="font-display-lg text-lg font-bold text-[#1E3F20] mb-2 leading-tight">{item.title[lang]}</h3>
                            <p className="font-body-md text-[13px] text-[#1b1c1c]/70 leading-relaxed line-clamp-3">{item.caption[lang]}</p>
                          </div>
                        </motion.div>
                      );
                    })}
          ...
          <section className="flex flex-col items-center pb-32 pt-16">
            ...
            <button className="group flex flex-col items-center gap-4 text-[#1E3F20] transition-all hover:scale-105">
              <span className="font-body-md text-[12px] font-bold uppercase tracking-[0.3em]">
                {lang === 'id' ? 'Muat Koleksi Lainnya' : 'Load More Collections'}
              </span>
      ...
      {/* Pop-up Modal "Kisah Dibalik Karya" */}
      <AnimatePresence>
        {selectedItem && (
          <motion.div 
            initial={{ opacity: 0 }}
          ...
          >
            ...
            >
              ...
              <div className="w-full md:w-1/2 flex flex-col justify-center">
                <p className="text-[12px] text-[#B2533E] font-bold uppercase tracking-[0.2em] mb-2">{selectedItem.koleksi[lang]}</p>
                <h2 className="font-display-lg text-3xl font-bold text-[#1E3F20] mb-4 leading-tight">{selectedItem.title[lang]}</h2>
